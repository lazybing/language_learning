...
int main()
{
    ...
    //insert elements in random order using differnet memeber functions
    ...
    if (status.second)
        cout << "4 inserted as element " << distance(coll.begin(), status.first) + 1 << endl;
    else
        cout << "4 already exists" << endl;

    //assign elements to another set with ascending order
    set<int> coll2(coll.cbegin(), coll.cend());
    ...
    //remove all elements with value5
    int num;
    num = coll2.erase(5);
